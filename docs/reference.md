...
| Task | `tail`, `logs`, `show` |
| Composer | `reply`, `re` |
| Actions | `tap`, `cancel`, `sleep` (a fixed choice of 15m, 30m, 1h, 2h, 4h or 8h), `done`, `discard`, `undone`, `undiscard`, `pin`, `unpin`, `max`, `unmax`, `switch-backend`, `rename`, `branch`, `kill`, `rm` |
| New task | `add` |
...
`reply -t`, `unread` and `alias`, to keep the actions sheet short. `workdir` and the
