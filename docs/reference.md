...
Closing a task gives it a permanent number, shown before the alias in `ilan ls -a` and `ilan search`, and at the head of every `ilan latest` line:
...
```bash
$ ilan branch fix-bug -d "Try the OAuth2 flow instead, as a spike"
Branched xxx-owl-naps-dune from fix-bug.
Burnable (xxx-…): ilan done / ilan discard will delete it. Rename it to keep it.
```

The quick form also works for branches: `ilan branch fix-bug "Try the OAuth2 flow instead, as a spike"` sends the same assignment as `-d` and creates a burnable child. A parent alias works too, for example `ilan branch sd "Explain the last failure"`. Like `ilan add "…"`, the bare instruction takes no flags: use `-d` or `-f` when naming the child with `-n`, and do not mix assignment sources. An empty or whitespace-only assignment is refused. The child inherits the parent's context and backend through the normal branch path; the parent continues independently.

The child's name is the only thing that decides: an unnamed branch off a keeper parent is burnable, and a `-n`-named child of a burnable parent is not.
...
- You can name a task `xxx-…` yourself with `-n` (on `add` or `branch`); nothing about a generated name is special.
...
- A burnable task's agent is killed first if it is still `WORKING` or `SLEEPING`, along with its tmux sessions.

## Quick side questions

Use `ilan btw TASK "question"` to ask about an existing task without taking over its earlier requests or running jobs. `TASK` can be a name or alias:

```bash
ilan btw sd "Why did you choose this learning rate?"
# If sd is the alias of train-model, the child is xxx-train-model-btw.
```

The child inherits the parent's context through the normal branch path. `btw` requires exactly two positional arguments: the parent name or alias and the instruction. It rejects every flag, including `-n`, `-d`, `-f`, and `--help`. `ilan task btw` is the canonical spelling. Empty assignments are refused before the suffix is added. Parent tail references (`@N`) expand before the suffix, just as on `branch`. Use `--` before the positional arguments if the instruction starts with a dash.

A side question does **not** keep the parent's [reasoning level](#commands) the way an ordinary branch does: it always starts at `low`, whatever the parent runs at, so a quick question about a `max` task does not run at `max` too. Its very first answer is already at `low`. Raise it with `ilan level CHILD max` if the question turns out to be hard; the parent's own level is untouched either way. `btw` reports the level it started at: `Branched xxx-train-model-btw from train-model. Reasoning level: low.`

The child's name is `xxx-<full-parent-name>-btw`. The full current parent name is used, including when you pass an alias: both `ilan btw train-model "Why?"` and `ilan btw sd "Why?"` create `xxx-train-model-btw` when `sd` names `train-model`. If that name is taken, the server tries `xxx-train-model-btw-2`, then `xxx-train-model-btw-3`, and so on, choosing the first free name. Simultaneous questions also get distinct names. Existing tasks stay intact; names freed by burning or renaming a child can be reused. Ordinary `ilan branch` still generates a random burnable name when `-n` is omitted.

`btw` also appends this fixed suffix to the child's assignment after a blank line and `---`:

> This is a quick side question in a separate branch. Focus only on the user's instruction above; use the inherited conversation as background. Earlier requests and ongoing jobs remain the parent task's responsibility. Do not resume that work or continue monitoring, managing, or waiting for those jobs unless the instruction above explicitly asks you to. Answer the current request directly, then stop.

The parent continues independently. The suffix guides the child agent's scope; it does not change job ownership or permissions. Read the answer with `ilan tail CHILD`, then use `ilan done CHILD` to burn it, or rename it to keep it.
...
| `ilan task check-model NAME` | Print the model name (e.g. `claude-opus-5-5`) that generated the last assistant message in the task's Claude Code session log |
| `ilan task tail NAME [-n N]` | Show the last assistant message together with the user prompt that elicited it and any user replies after it; with `-n N`, show history containing the final N assistant messages (user messages do not consume the limit); ends with lines naming the model (plus reasoning effort, paying account and, for API-key spends, cost) and that message's input/output/cached-input token counts. `-m`/`--md` renders the assistant messages as Markdown and `--no-md` prints them as plain text, each overriding the `markdown` config key for this invocation |
| `ilan task reply NAME ["msg"]` | Send a reply to an agent (omit message to show tail; `-n N`, `-m`/`--md`, `--no-md` and `--line-number`/`--no-line-number` shape that tail exactly as on `ilan task tail`, and the two on/off pairs are refused when a message is sent, since there is no tail then). Pass `--max` to switch the task to its backend's [max model](#max-models-ilan-max--ilan-unmax) before posting the reply (persists for all subsequent messages), or `--unmax` to reset the model to the config default before posting. Pass **`--level LEVEL`** (`low`, `medium` or `max`, case-insensitive) to set the task's [reasoning level](#commands) starting from the answer to this reply: the level travels with the reply in the same request, so the server sets it only once the reply is accepted — a declined `reply -t` confirmation or a refused reply leaves the level as it was — and before the agent is resumed, so the turn that answers this message already runs at the new level (on a `WORKING` task, the interrupted turn keeps the level it started with). Like `--max`, the level persists for every turn after; it needs a message (`ilan level` changes the level on its own), combines with `-e` and `-t`, and is refused with `-u`. The CLI confirms it with `Reasoning level set to max (effort: xhigh), starting with the answer to this reply.`, and warns if a server older than the flag ignored it. Pass `-t DURATION` (same format as `ilan sleep`, minimum 20 minutes, e.g. `-t 1h`) to also re-send `"msg"` to the task every DURATION: the task's name is highlighted with a `(responding every 1h)` suffix in `ilan ls` / `ilan dashboard`, its status renders as a light-purple `AGENT_IN_LOOP` wherever it would otherwise read `AGENT_FINISHED` or `NEEDS_ATTENTION` (the stored status is unchanged — the cycle, not you, is what re-prompts the agent), and the cycle runs until your next human message to the task (`reply`/`tap`/`cancel`/`sleep` — each asks `[y/n]` before ending it; confirming a new `reply -t` replaces the cycle) or until the task is killed or marked `DONE`/`DISCARDED`. Pass `-t DURATION` **without a message** on a task that is already looping to change its cadence: the cycle's own message is re-sent at once, as if the timer had just fired, and then every DURATION from then on (`Will re-send it every 2h (was 1h) until the next human reply.`); the message itself is unchanged. On a task that is not looping this is refused, since there is no message to re-send — give one to start a cycle. Pass **`-e`** (`--editor`) to write the message in the [`editor`](#configuration-keys) from your config rather than on the command line, for a reply too long or too structured to quote into a shell. `-e` takes no message of its own — passing one as well is rejected — and the buffer opens empty, since a reply is new text rather than an edit of something the task already carries. What you save is sent with leading and trailing whitespace stripped, so the newline your editor leaves behind is not part of the message, while the layout inside it is kept as written. Nothing is sent unless the edit produced text: leaving the buffer empty (or all whitespace) reports `Empty reply; nothing sent.`, and an editor that exits non-zero (`:cq` in vim, or a crash) reports that too — both exit 1, so `ilan re t -e && …` cannot run on as though a reply had been posted. Everything knowable beforehand is checked before the editor opens — that the `editor` config value names something installed and on your `PATH`, that the task exists, that a `-t` duration parses, that `--max`/`--unmax` were not both given — so a mistake never costs you a reply you have already typed. `-e` combines with `--max`, `--unmax` and `-t`, which treat it as the message they require. Pass **`-u`** (`--update`) on a task with an active `-t` cycle to change the message it re-sends: your `editor` opens prefilled with the current message, and what you save replaces it from the next re-send on. The cadence and the time of the next re-send are unchanged, and nothing is sent now; the log picks up the new text when the timer next delivers it. `-u` is refused on a task that is not looping (a cycle is started with `reply -t`, which needs a cadence), takes no message of its own, and combines with none of the other reply flags. Leaving the buffer as it was writes nothing and reports the prompt unchanged; emptying it keeps the current message and exits 1, since a cycle with nothing to send is not a cycle (end one with any human reply), as does an editor that exits non-zero. The usable-editor and real-task checks run before the editor opens, and the server checks again when the edit lands that the task is still looping, since the cycle may have ended while the editor was open. **`-h`** (as well as `--help`) prints how these flags fit together — sending, reading the tail, looping, the editor and the model switch — followed by worked examples |
| `ilan task tap NAME` | Ask for a status update (interrupts `WORKING` / `SLEEPING` agents; re-prompts `AGENT_FINISHED`/`NEEDS_ATTENTION`/`ERROR` tasks) |
| `ilan task cancel NAME` | Retract the message you last sent to a task — replies telling the agent that message was a mistake, that it should be ignored, and that any work already started on it should stop immediately. Accepts the same statuses as `tap` (`WORKING`/`SLEEPING`/`AGENT_FINISHED`/`NEEDS_ATTENTION`/`ERROR`) |
| `ilan task sleep NAME DURATION` | Re-prompt a `NEEDS_ATTENTION` / `AGENT_FINISHED` task to sleep for DURATION and report back. DURATION is an integer or decimal with an optional unit suffix — no whitespace — e.g. `300`, `300s`, `5m`, `2h`, `1.5h`. Units: `s`/`sec`/`second`/`seconds`, `m`/`min`/`mins`/`minute`/`minutes`, `h`/`hr`/`hrs`/`hour`/`hours`; bare numbers are seconds. The task enters `SLEEPING`. In the full `ilan ls` / `ilan dashboard` Status column, a dark-blue label and ten-cell bar show elapsed time against the total (for example, `SLEEPING █████░░░░░ 2m30s / 5m`); elapsed time is clamped at the requested duration. The web app shows the same progress beside the status. |
...
| `ilan task notes NAME [["note"] \| -a TEXT \| -c \| -e]` | Write the free-form reminder shown in light-green italics beneath the task's alias and name in `ilan ls` / `ilan dashboard` — what the task is about, so a listing full of names still means something a week later. `ilan task note` is the same command. The note **replaces** whatever the task carried before (correcting one is just writing it again), and an empty or all-whitespace note clears it. Three flags cover the other things you want to do: **`-a TEXT`** (`--append`) adds `TEXT` to the end of the note already there, separated by exactly one space — appending to a task with no note simply sets it, with no leading space; **`-c`** (`--clear`) removes the note; and **`-e`** (`--editor`) opens the note in the [`editor`](#configuration-keys) from your config, prefilled with the note the task already has, and saves whatever you leave in the buffer. **A bare `ilan notes NAME`, with no note and no flag, is the same as `-e`**: with nothing to set, add or clear, editing the note the task already has is the only thing left for the command to mean, so it opens the editor rather than reporting a usage error. An explicitly empty note (`""`) is still a note you gave, so it clears rather than opening the editor. `-c` and `-e` take no text of their own; passing some anyway, or combining any two of `-a` / `-c` / `-e`, or giving both a note and one of them, is rejected before the request is sent, since each of the four says what the note should become and any two of them contradict. Every string you supply is stripped of leading and trailing whitespace and newlines, on both sides of an append, so however you pad an argument the separator is one space; whitespace *inside* a note is left as you wrote it. Appending is done by the server under its store lock, so two appends both land rather than the second overwriting the first from a stale read. With `-e` the buffer holds nothing but the note — no instruction header, so there is no comment syntax to strip and a note may legitimately start with `#`. Closing the editor without changing anything writes nothing and reports the note as unchanged; emptying the buffer clears the note; and an editor that exits non-zero (`:cq` in vim, or a crash) leaves the note alone and exits 1, so an abandoned edit is never mistaken for an emptied one. `-e` needs an editor it can actually open: the `editor` config value is checked before the task is even fetched, and a blank one, or one that is not installed or not on your `PATH`, is an error naming it and pointing at `ilan config set editor <name>`. The default is `emacs`, which is only a guess on a machine that may not have it, so this is a real check rather than a formality. A note is capped at **256 characters**, measured after stripping and counted in characters rather than bytes. A longer one is **not** an error: it is cut down to its first 256 and saved, and the command says so (`Note was longer than 256 characters; kept the front and dropped the rest.`) so the trim is never silent. That is a ceiling rather than a target — a note using all of it fills about six lines of the `Notes` column on a wide window and ten on a narrow one. The front is what survives, since that is where a reminder says what it is about, and a cut that lands on a space leaves no trailing whitespace behind. On an `-a` append the cap applies to the **combined** note, so appending to a note that is already full leaves it as it was. The cap is applied by the server, so every client gets the same answer, and it is what keeps a note glanceable under the name instead of pushing the rest of the listing down the screen. A note **outlives the task's working life**: `done` drops a task's alias and `discard` drops its unread marker, but neither touches the note, and it survives `undone` / `undiscard` too — `ilan ls -a` is where a closed task's note is read. The one exception is a [burnable](#burnable-tasks-xxx-) `xxx-` task, which `done`/`discard` delete outright, note and all. A closed task can also still be annotated, unlike its alias, which is frozen. Unlike the `Status` cell's one-line summary the note is never regenerated, so nothing overwrites what you wrote. A branched child does not inherit its parent's note |
| `ilan task branch OLD [INSTRUCTION] [-n NEW] [-d "msg" \| -f FILE]` | Branch a new task from a name or alias, inheriting its full context (both tasks stay repliable and diverge from there). A bare `INSTRUCTION` creates a burnable child and takes no flags. Otherwise, exactly one of `-d` / `-f` supplies the child's first assignment; omit `-n` for a generated [burnable](#burnable-tasks-xxx-) name. The child is told that the inherited conversation is background context only. It keeps the parent's [reasoning level](#commands), and the confirmation names it (`Reasoning level: max`) |
| `ilan task btw OLD INSTRUCTION` | Create a burnable branch named `xxx-<full-parent-name>-btw`, resolving aliases to the full parent name and adding `-2`, `-3`, etc. if taken. Requires a positional instruction and rejects all flags. A fixed suffix asks the child to focus on the current instruction and leave earlier work and running jobs to the parent. See [Quick side questions](#quick-side-questions) |
| `ilan task kill NAME` | Kill a `WORKING` or `SLEEPING` agent, move task to `ERROR` |
...
| `ilan task undone NAME` | Move a `DONE` task back to `NEEDS_ATTENTION`, which re-activates it and so moves it to the bottom of `ilan ls` / `ilan dashboard` — see [Listing order](#listing-order). `NAME` may be the task's [number](#task-numbers) (`ilan undone 1`) |
| `ilan task undiscard NAME` | Move a `DISCARDED` task back to `NEEDS_ATTENTION`, which re-activates it and so moves it to the bottom of `ilan ls` / `ilan dashboard` — see [Listing order](#listing-order). `NAME` may be the task's [number](#task-numbers) (`ilan undiscard 2`) |
...
| `ilan task level NAME LEVEL` | Set the task's reasoning level to `low`, `medium` or `max` (case-insensitive; any other value is rejected). The level is stored on the task and translated at every spawn for the backend the task is on then: `low` and `medium` pass through unchanged, and `max` becomes `--effort max` on `claude` and `-c model_reasoning_effort="xhigh"` on `codex`, the top of each ladder. `ilan attach` resumes the session at the same effort. New tasks start at `low` (so do tasks stored before levels existed); a branch keeps its parent's level, except that an [`ilan btw`](#quick-side-questions) side question always starts at `low`; a backend switch keeps it. Takes effect on the task's next agent spawn. Shown in the `Reasoning` column of `ilan ls` / `ilan dashboard` |
...
| `ilan notes NAME [["note"] \| -a TEXT \| -c \| -e]` | `ilan task notes NAME [["note"] \| -a TEXT \| -c \| -e]` |
| `ilan note NAME [["note"] \| -a TEXT \| -c \| -e]` | `ilan task notes NAME [["note"] \| -a TEXT \| -c \| -e]` |
| `ilan branch OLD [-n NEW] -d "msg"` | `ilan task branch OLD [-n NEW] -d "msg"` |
| `ilan branch OLD "instruction"` | `ilan task branch OLD "instruction"` — quick burnable branch, no flags |
| `ilan btw OLD "instruction"` | `ilan task btw OLD "instruction"` — quick burnable side question, no flags |
...
| `ilan level NAME LEVEL` | `ilan task level NAME LEVEL` |
...
Neither listing shows a `Created` column, at any terminal width. Creation time is a fact about a task you look up once, not one you scan a table for, and at 20 characters it was the widest thing on either listing that nobody reads twice — `ilan info NAME` reports it, to the second, along with everything else about that task. The 20 characters it held go to `Name` and `Status`, ten each, so the two prose columns between them reclaim all of it rather than leaving it blank to the right of the table.

The column order is `Name` / `Status` / `Reasoning`. `Reasoning` shows the whole ladder, `low ⋅ medium ⋅ max`, with the task's [reasoning level](#commands) (set with `ilan level`) in its colour — `low` green, `medium` yellow, `max` red — and the other two in grey, so the expensive tasks stand out. `ilan ls --concise` prints only the active level, in the same colour, at the end of each line. A task's note has no column of its own: it is rendered in light-green italics on its own line **beneath the alias and name**, in the `Name` cell — it belongs with the name because it says what the task is *for*, where the `Status` cell beside it says what the agent just did. The alias carries one more fact: an ordinary task's is written `(gk)`, and **once the task is maxed it is written `[GK]`** — capitals inside square brackets, in red (see [Max models](#max-models-ilan-max--ilan-unmax)); no `FABLE` / `ASTRA` word is printed in the listing any more.

`Reasoning` is pinned to 18 characters, exactly the width of the ladder, and `Name` and `Status` share the rest **equally**. Both are prose columns, the note under the name and the summary under the status label, so neither has a claim on more room; because the split is proportional, writing or lengthening a note never moves a column, which matters on a view that redraws once a second. In `ilan ls` both prose columns are sized to their contents and share one cap of **52** characters, so a long note or summary folds within its cell instead of pushing the table off the right edge, the two come out equal whenever both are full, and a listing of short names and short statuses stays compact. The cap is 52 rather than the 42 it was while `Created` was still a column because it absorbed half of that column each: the dashboard's prose columns are proportional and widened on their own the moment the pinned column left the table, but a content-sized column stops at its cap, and one that had not moved would have left the freed characters as blank terminal. On a window too small for that — below 132 columns, the two caps plus the `Reasoning` column plus the rules Rich draws between them — the window itself is the limit and both listings simply shrink to fit. `ilan ls --concise` does not show notes — that view is one greppable line per task, and free-form prose would break its shape.
...
The `Branch tree` section of [`ilan info`](#task-info) draws the whole tree the task belongs to, starting from its outermost ancestor, with the task you asked about marked `← this task`. There is no command of its own: a tree read on its own always led to the same next question — what *is* that task, and what has it been doing — which is what the rest of that view answers. Terminal tasks are always included, so the tree doesn't fragment when a middle task is marked `DONE`:
...
caches the reasoning effort the task's level resolved to. With `api-key-mode` enabled, a non-empty matching
...
Each new assistant comment mirrored to a GitHub Gist also records the task alias
that was active when the message was generated:

```
*(message is generated by claude-opus-5, task alias: `ds`, effort: xhigh, budget: Team)*
```

The alias is stored with the message rather than looked up during the
asynchronous upload, so changing the task's alias later does not rewrite its
history. Comments created before alias tracking omit this field.
...
`ilan task branch` and `ilan task btw` (including their shorthands) do the same expansion on their
first assignment, looking up the cached tail of the parent task. Both accept a
positional instruction; `branch` also accepts `-d "..."` or `-f FILE`. Run
`ilan tail parent` and then `ilan btw parent "explain @12"`, or
...
— and in red, the colour the tag itself was drawn in while it still appeared
in the listing and the colour `ilan max` still confirms it in; those two
things are the only mark the listing gives it, and the tag itself is not
printed there. Type the alias either way: `ilan notes GK` and `ilan notes gk`
...
its status — `Reasoning: low ⋅ medium ⋅ max` — listing all three levels the way the
`Reasoning` column of `ilan dashboard` does: only the task's own level is coloured (green
`low`, yellow `medium`, red `max`), and the other two are grey. The line is on an expanded
...
the task list and the task's conversation page and advances whenever the view refreshes.
...
- **Catch-up on switch.** `ilan task switch-backend` is *lazy* — it only
  rewires which backend the task uses on its next spawn. A running (`WORKING`
  or `SLEEPING`) task cannot be switched: the command warns and does nothing,
  so an in-flight
...
`ilan sleep` moves a `NEEDS_ATTENTION` or `AGENT_FINISHED` task to `SLEEPING`
while its agent waits. When the wait ends, the agent reports back into
`AGENT_FINISHED` or `NEEDS_ATTENTION`; a human reply, tap, or cancel interrupts
the wait and resumes ordinary `WORKING`.
...
- **The task name is the link**: in `ilan ls` (including `--concise`), `ilan search`, and `ilan dashboard`, a mirrored task's name is underlined and hyperlinked to its Gist, so a click in any terminal that supports OSC 8 links opens the conversation. A task with no Gist yet is left plain, which is how the two are told apart at a glance.
