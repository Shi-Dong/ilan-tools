...
The only difference from `branch` is this fixed suffix, appended to the child's assignment after a blank line and `---`:
...
| `ilan task btw OLD [INSTRUCTION] [-n NEW] [-d "msg" \| -f FILE]` | Same branch behavior and options, with a fixed suffix asking the child to focus on the current instruction and leave inherited work and running jobs to the parent. See [Quick side questions](#quick-side-questions) |
