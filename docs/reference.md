...
|---|---|
| List | `ls`, `ls -a`, `search`; an expanded card shows the task's note under its summary, and each card also offers `tap`, `done`, `notes` (its sheet has a Clear button, as `notes -c`), and — on a closed task — `undone` / `undiscard` |
| Task | `tail`, `logs`, `show` |
| Composer | `reply`, `re` |
