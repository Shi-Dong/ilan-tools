...
The child inherits the parent's context through the normal branch path. Like `ilan branch`, the bare form creates a burnable child and takes no flags; `-d` and `-f` also work, optionally with `-n` to keep a named child. `ilan task btw` is the canonical spelling. Empty assignments are refused before the suffix is added. Parent tail references (`@N`) expand before the suffix, just as on `branch`.

Without `-n`, the child's name is `xxx-<parent-name>-btw`. The full current parent name is used, including when you pass an alias: both `ilan btw train-model "Why?"` and `ilan btw sd "Why?"` create `xxx-train-model-btw` when `sd` names `train-model`. The same naming rule applies with `-d` or `-f`. An explicit `-n` overrides it. If the derived name already exists, the command reports the usual name-collision error without changing either task; burn that child with `ilan done`, or rename it, before asking another side question under the same name. Ordinary `ilan branch` still generates a random burnable name when `-n` is omitted.
...
| `ilan task btw OLD [INSTRUCTION] [-n NEW] [-d "msg" \| -f FILE]` | Branch with a default name of `xxx-<parent-name>-btw`, resolving aliases to the full parent name. An existing name is refused; `-n` overrides the default. A fixed suffix asks the child to focus on the current instruction and leave earlier work and running jobs to the parent. See [Quick side questions](#quick-side-questions) |
...
| `ilan btw OLD "instruction"` | `ilan task btw OLD "instruction"` — quick side question; also accepts branch's `-d` / `-f` and `-n` forms |
