...
Both `ilan ls` and `ilan dashboard` adapt to the terminal width: on a window narrower than 120 columns they drop the `Created` column so the remaining `Name` / `Status` / `Notes` / `Last Changed` columns stay legible, and the `Notes` column falls back from 26 characters to 20 — below the threshold there is no slack left in `Name` and `Status` to take it from.
