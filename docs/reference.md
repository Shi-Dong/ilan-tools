...
Both `ilan ls` and `ilan dashboard` adapt to the terminal width: on a window narrower than 120 columns they drop the `Created` column, leaving `Name` / `Status` / `Last Changed`, and the 20 characters it held go to `Name` and `Status` — ten each, so the two prose columns between them reclaim all of it rather than leaving it blank to the right of the table.
