...
Both `ilan ls` and `ilan dashboard` adapt to the terminal width: on a window narrower than 120 columns they drop both the `Created` **and** the `Notes` column, leaving `Name` / `Status` / `Last Changed`. The note is not lost — it moves inside the `Status` cell, on its own line beneath the one-line summary, keeping its light red so it still reads as your note rather than more of the agent's summary. That is what makes a narrow window work: two prose columns side by side leave nothing for the task name, one prose column stacked does not.

The column order is `Name` / `Status` / `Notes` / `Created` / `Last Changed`. `Notes` sits beside `Status` on purpose: the agent's one-line summary of what it just did and your own note on why the task exists answer the same question, so reading one after the other should not mean crossing two timestamps.

The `Notes` column is in a light red and carries whatever you wrote with [`ilan notes`](#commands) — see the `ilan task notes` row above. It is always there, whether or not any task has a note, and it is a fixed width: a column that resized itself to its contents would shift every column to its left each time a note was written, cleared, or lengthened, and the dashboard redraws once a second. `Created` and `Last Changed` are pinned to 15 characters for the same reason and to help pay for it — 15 fits every stamp they render (`Today 13:30 PDT`, `09-05 13:30 PDT`) on one line, folding only the `Yesterday …` form. `Status` and `Notes` are sized **against each other at roughly 3:2** rather than independently, since they hold the two answers to what a task is about — the agent's summary of what it just did, and your own note. `Status` takes the larger share because it carries a label and a duration on top of its prose. In `ilan ls` that is already the case at 38 characters against 26, so nothing there changed: `Status` is capped rather than pinned, since `-a` suppresses the summary and a listing of short statuses should not reserve the room a summary would have needed. In the dashboard `Status` used to grow with the terminal while `Notes` stayed pinned, reaching 95:26 on a 240-column window; now the two are given the room they held between them and it is split 3:2 instead. That total is conserved deliberately, so the width `Notes` gains comes out of `Status` and out of nothing else — `Name` and the two timestamp columns render at exactly the widths they did before. `ilan ls --concise` does not show notes — that view is one greppable line per task, and free-form prose would break its shape.
