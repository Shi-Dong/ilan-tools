...
Both listings end with a `Notes` column, in a light red, carrying whatever you wrote with [`ilan notes`](#commands) — see the `ilan task notes` row above. The column is always there, 20 characters wide, whether or not any task has a note: a column that resized itself to its contents would shift every column to its left each time a note was written, cleared, or lengthened, and the dashboard redraws once a second. `Created` and `Last Changed` are pinned to 15 characters for the same reason and to pay for the new column — 15 fits every stamp they render (`Today 13:30 PDT`, `09-05 13:30 PDT`) on one line, folding only the `Yesterday …` form. `Notes` also survives the narrow-window rule: below 120 columns `Created` is still the column that goes, since a note you asked for is worth more than a creation timestamp. Below about 70 columns the fixed columns stop yielding and the flexible `Name` / `Status` pair gets squeezed instead. `ilan ls --concise` does not show notes — that view is one greppable line per task, and free-form prose would break its shape.
