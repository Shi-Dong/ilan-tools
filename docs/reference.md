...
A sleeping task has its own `SLEEPING` state, rendered in a darker blue than `WORKING`. Its `Name` cell keeps the requested duration beside the task as `(sleeping for 5m)`, in the same dark blue. Its `Status` cell adds a ten-cell progress bar and exact elapsed/total clock, such as `█████░░░░░ 2m30s / 5m`. The clock advances on each dashboard refresh and stops at the requested total rather than running past 100%.
