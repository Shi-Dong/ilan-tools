...
class TestHelpers:
    ...
    def test_format_comment_assistant_with_task_alias(self) -> None:
        entry = LogEntry(
            role="assistant", content="done", timestamp="",
            model="claude-opus-5", task_alias="ds", effort="xhigh",
            budget="Team",
        )
        out = format_comment(entry)
        assert out == (
            "**Assistant**\n\ndone\n\n"
            "*(message is generated by claude-opus-5, task alias: ds, "
            "effort: xhigh, budget: Team)*"
        )

    def test_format_comment_omits_token_usage(self) -> None:
        entry = LogEntry(
            role="assistant",
            content="done",
            timestamp="",
...
class TestSyncTask:
    ...
    def test_comment_keeps_alias_from_message_generation(
        self, store: Store, syncer: GistSyncer, fake_gh: _FakeGitHub
    ) -> None:
        task = Task(name="t1", prompt="p", alias="ds")
        store.put_task(task)
        store.append_log(
            "t1",
            "assistant",
        ...
        assert len(fake_gh.comments) == 1
        assert "task alias: ds" in fake_gh.comments[0][1]
        assert "task alias: dh" not in fake_gh.comments[0][1]
