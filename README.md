...
| `ilan notes NAME "note"` | Write a reminder of what the task is about, shown in a light-red `Notes` column in `ilan ls` and `ilan dashboard`. `ilan note` is the same command; `""` clears the note. |
