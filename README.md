...
- **Existing tasks are back-filled**: the first new message on any pre-existing task creates its Gist and posts the entire prior history in order. When ilan first starts with task names enabled in Gist descriptions, it also refreshes existing mirrored tasks in the background so their browser-tab titles are upgraded without waiting for another message.
