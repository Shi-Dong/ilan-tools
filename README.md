...
| `ilan task tail NAME` | Show the last assistant message together with the user prompt that elicited it and any user replies after it; ends with a line naming the model (and the reasoning effort, paying account and, for API-key spends, the cost) that generated the last assistant message |
