...
| `ilan notes NAME ["note"] [-a TEXT] [-c] [-e]` | Write a reminder of what the task is about, shown in a light-red `Notes` column in `ilan ls` and `ilan dashboard` — or, on a window too narrow for that column, under the summary in the `Status` cell — and under the summary on the web app's task list. `-a` adds to the note already there (separated by a space), `-c` clears it, `-e` edits it in your configured editor. The note stays with the task after it is closed; anything past 256 characters is cut to fit. `ilan note` is the same command. |
