...
| `ilan task tail NAME` | Show the last assistant message together with the user prompt that elicited it and any user replies after it; ends with lines naming the model (plus reasoning effort, paying account and, for API-key spends, cost) and that message's input/output/cached-input token counts |
...
These are the counters for that assistant message's complete backend invocation,
not the task's running totals. Codex's native input counter includes cached input,
so ilan subtracts its cached count to keep `input` disjoint from `cached input`,
matching Claude's reporting. Older messages have no per-message counters and omit
the line. Token usage is shown only by `ilan tail`; it is not added to GitHub Gist
comments.
