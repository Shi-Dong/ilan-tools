...
| `ilan task cancel NAME` | Retract the message you last sent to a task — replies telling the agent that message was a mistake, that it should be ignored, and that any work already started on it should stop immediately. Accepts the same statuses as `tap` (`WORKING`/`AGENT_FINISHED`/`NEEDS_ATTENTION`/`ERROR`). This is a *reply*, not a kill: a `WORKING` agent sees it on its next turn, so use `ilan task kill` if you need to stop one outright |
