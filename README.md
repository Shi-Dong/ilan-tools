...
| Command | What it does |
|---|---|
| `ilan max NAME` / `ilan unmax NAME` | Run the task on its backend's max model — Fable (`claude-fable-5-1`) on `claude`, Astra (`gpt-6-astra`) on `codex` — or return to the configured default. In `ilan ls` and `ilan dashboard` a maxed task's alias is written `[GK]` — capitals in square brackets — where an ordinary task's is `(gk)`. Takes effect on the next reply. |
