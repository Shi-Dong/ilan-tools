...
A task is `WORKING` from the second it is created. `ilan sleep` moves an idle task to `SLEEPING`. The full terminal listing keeps `(sleeping for 5m)` beside its name and, like the web app, shows elapsed time against the requested duration. Any reply to a finished, blocked, or errored task re-spawns its agent at once. Agents report their own state: Ilan asks every prompt to end with a `[STATUS: DONE]` or `[STATUS: NEEDS_ATTENTION]` marker.
...
| Command | What it does |
|---|---|
| `ilan reply NAME ["msg"] [--max \| --unmax] [-t DURATION] [-e] [-u]` | Send a message; `ilan re` is the same. Without a message it shows the tail. `-t 1h` re-sends the message every hour until you write to the task again; `-t 30m` on its own, on a looping task, switches it to every 30 minutes and re-sends its message at once. `-e` writes the message in your configured editor instead of on the command line, and sends nothing if you leave the buffer empty. `-u` opens the message a looping task re-sends in your editor, prefilled, so you can change it; the new text goes out from the next re-send on, on the same schedule. |
